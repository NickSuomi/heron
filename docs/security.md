...
- **Model text is posted as plain text.** Summaries, finding titles and bodies, finding paths, limitations, vendor error text and provenance table cells are never read as Markdown. Heron changes them as follows:
  - Every ASCII punctuation character (``!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~``) gets a backslash in front of it and an invisible word joiner (U+2060) after it. The backslash stops Markdown: no code span, code block, emphasis, link, image, heading, list, quote, table, HTML tag, HTML comment or entity can open, and no line can start with the `/` that GitLab runs as a quick action. The joiner stops GitLab's text filters: no mention, issue, merge request, label, milestone, epic, snippet or cross-project reference forms, and no URL is linked. A bare commit hash has no punctuation and stays as written, so GitLab may still link it to that commit in the same project; that notifies nobody.
  ...
  - This is the cost of the rule: code and URLs that the model quotes show as literal, unlinked text, and text copied from the note carries the invisible joiners. Heron does not try to find where code or links start and end, because three earlier attempts to mirror GitLab's parsing disagreed with it and let mentions, references or HTML through.
  - `test/report-render.test.ts` renders reports built from hostile text with markdown-it and checks that the text adds only paragraphs and line breaks, that no reference sigil is left without a joiner, and that no line starts with `/`. markdown-it stands in for GitLab's renderer; GitLab itself is not tested.
